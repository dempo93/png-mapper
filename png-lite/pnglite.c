...
static int file_read_ul(png_t* png, unsigned* out)
{
    ...
    if (file_read(png, buf, 1, 4) != 4) return PNG_FILE_ERROR;
...
static int file_write_ul(png_t* png, unsigned in)
{
    ...
    if (file_write(png, buf, 1, 4) != 4) return PNG_FILE_ERROR;
...
static int png_read_ihdr(png_t* png)
{
    ...
    if (file_read(png, ihdr, 1, 13 + 4) != 13 + 4) return PNG_EOF_ERROR;
#if DO_CRC_CHECKS
    ...
    if (orig_crc != calc_crc) return PNG_CRC_ERROR;
#else
    file_read_ul(png);
#endif
    ...
    if (png->color_type == PNG_INDEXED) return PNG_NOT_SUPPORTED;

    if (png->depth != 8 && png->depth != 16) return PNG_NOT_SUPPORTED;

    if (png->interlace_method) return PNG_NOT_SUPPORTED;
...
int png_open_read(png_t* png, png_read_callback_t read_fun, void* user_pointer)
{
    ...
    if (!read_fun && !user_pointer) return PNG_WRONG_ARGUMENTS;

    if (file_read(png, header, 1, 8) != 8) return PNG_EOF_ERROR;

    if (memcmp(header, "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A", 8) != 0) return PNG_HEADER_ERROR;
...
int png_open_write(png_t* png, png_write_callback_t write_fun, void* user_pointer)
{
    ...
    if (!write_fun && !user_pointer) return PNG_WRONG_ARGUMENTS;
...
int png_open_file(png_t* png, const char* filename) { return png_open_file_read(png, filename); }
...
static int png_init_deflate(png_t* png, unsigned char* data, int datalen)
{
    ...
    if (deflateInit(stream, Z_DEFAULT_COMPRESSION) != Z_OK) return PNG_ZLIB_ERROR;
...
#if USE_ZLIB
    memset(stream, 0, sizeof(z_stream));
    if (inflateInit(stream) != Z_OK) return PNG_ZLIB_ERROR;
#else
    memset(stream, 0, sizeof(zl_stream));
    if (z_inflateInit(stream) != Z_OK) return PNG_ZLIB_ERROR;
#endif
...
static int png_inflate(png_t* png, char* data, int len)
{
    ...
    if (stream->avail_in != 0) return PNG_ZLIB_ERROR;
...
static int png_read_idat(png_t* png, unsigned firstlen)
{
    ...
    if (type == *(unsigned int*)"IEND") result = PNG_DONE;
...
static int png_process_chunk(png_t* png)
{
    ...
    if (file_read(png, &type, 1, 4) != 4) return PNG_FILE_ERROR;

    if (type == *(unsigned int*)"IDAT") /* if we found an idat, all other idats should be followed with no other chunks
                                           in between */
    {
        png->png_datalen = png->width * png->height * png->bpp + png->height;
        png->png_data = png_alloc(png->png_datalen);

        if (!png->png_data) return PNG_MEMORY_ERROR;
...
static void png_filter_up(int stride, unsigned char* in, unsigned char* out, unsigned char* prev_line, int len)
{
    ...
    if (prev_line)
    {
        for (i = 0; i < len; i++) out[i] = in[i] + prev_line[i];
    }
    else
        memcpy(out, in, len);
}
...
static int png_filter(png_t* png, unsigned char* data) { return PNG_NO_ERROR; }
...
static int png_unfilter(png_t* png, unsigned char* data)
{
    ...
    while (pos < png->png_datalen)
    {
        ...
        switch (filter)
        {
            ...
            case 2: /* up */
                if (outpos)
                    png_filter_up(stride, filtered + pos, data + outpos, data + outpos - (png->width * stride),
                                  png->width * stride);
                else
            ...
            case 3: /* average */
                if (outpos)
                    png_filter_average(stride, filtered + pos, data + outpos, data + outpos - (png->width * stride),
                                       png->width * stride);
                else
            ...
            case 4: /* paeth */
                if (outpos)
                    png_filter_paeth(stride, filtered + pos, data + outpos, data + outpos - (png->width * stride),
                                     png->width * stride);
                else
...
char* png_error_string(int error)
{
    switch (error)
    {
        ...
        case PNG_WRONG_ARGUMENTS:
            return "Wrong combination of arguments passed to png_open. You must use either a read_function or supply a "
                   "file pointer to use.";
        default:
